#ifndef Aurora_h
#define Aurora_h
// ESP32 Optimized Version - HardwareSerial Only
// Removed SoftwareSerial dependency for improved performance
#include <Arduino.h>
...
class Aurora {
    private:
		...
		// ESP32 uses HardwareSerial only

		struct SerialWrapper{
		  template< typename T >
		  void begin( T &t, int baud ){
			  DEBUG_PRINTLN("Begin ESP32 HardwareSerial");
			  t.setTimeout(500);
			  t.begin(baud);
			  stream = &t;
		  }
		...
		SerialWrapper serialDef;

        byte inverterAddress = 0;
        byte serialCommunicationControlPin;

        // ESP32 HardwareSerial specific variables
